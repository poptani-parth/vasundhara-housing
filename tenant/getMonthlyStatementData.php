...
if (!$searchMonth) {
    $response['message'] = 'Month parameter is missing.';
...
try {
    ...
    $sql_payments = "
        SELECT p.payment_id, p.amount, p.payment_date, p.payment_period, p.remark, pr.pro_nm AS property_name
        FROM payments p
        JOIN properties pr ON p.property_id = pr.pro_id
        WHERE p.tenant_id = ? AND DATE_FORMAT(p.payment_period, '%Y-%m') = ?
        ORDER BY p.payment_date ASC
    ";
    $stmt_payments = $conn->prepare($sql_payments);
    $stmt_payments->bind_param("is", $userId, $searchMonth);
    ...
    $response['data'] = [
        ...
        'payments' => $payments,
        'period' => date('F Y', strtotime($searchMonth . '-01'))
    ];
